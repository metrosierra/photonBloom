...
for target_dir in targets:
    data_dir = 'data/photon2/' + target_dir
    ...
    for dossier_index in range(2):
        ...
        channel1fixed, channel2fixed = deli.data_crop(data, 5e12)
        ...
        hi3 = plt.hist(aggregate, density = True, bins = np.arange(-0.5, 5.5), label = '{} Data photon{} statistics'.format(target_dir[:-1], multiplex*2))
        ...
        poisson = [mathy.poissonian(mean, x) for x in poinput]
        plt.plot(poinput, poisson, label = 'Theoretical Poissonian ($\lambda$ = {})'.format(mean))
        plt.legend()
